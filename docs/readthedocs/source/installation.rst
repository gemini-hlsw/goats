...
.. note::
   ...
   Miniforge is preferred over Miniconda because it defaults to using the community-driven **Conda-Forge** repository, which provides more up-to-date and widely maintained packages. Additionally, Miniconda is developed by Anaconda, Inc., which enforces licensing restrictions for commercial use. Miniforge eliminates these legal concerns, making it a more flexible and widely accepted alternative.
...
1. Create the GOATS conda environment:
   ...
   .. note::
      **macOS M1/M2 ARM Users:** Some dependencies do not support ARM architecture. You must include the ``--platform osx-64`` flag when creating the environment to ensure compatibility:
...
Currently, DRAGONS does not support macOS ARM architecture. To ensure compatibility, use the ARM version of Miniforge but include the ``--platform osx-64`` flag when creating the environment:
...
Once the environment is created, run GOATS normally:

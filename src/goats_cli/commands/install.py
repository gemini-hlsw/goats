...
def install(
    ...
    ci: Annotated[
        ...
        typer.Option(
            ...
            "-c",
            help=(
                "Enable non-interactive mode for CI pipelines. "
                "Skips prompts, including superuser creation."
            ),
        ),
    ] = False,
):
    ...
    # Run copier to set up the project.
    ...
    output.section("Creating Superuser")
    env = os.environ.copy()
    ...
    try:
        subprocess.run(
            [str(manage_file), "createsuperuser"],
            check=True,
            env=env,
        )

...
def fail(msg: RenderableType) -> None:
    ...
    msg : RenderableType
        Error message text.
    """
    error_console.print(f"[bold red]✖[/] {msg}")
...
def procedure_steps(steps: list[RenderableType]) -> None:
    ...
    """
    for step in steps:
        console.print(f"  [bold white]•[/] {step}")
...
def info_table(items: dict[str, str]) -> None:
    ...
    for key, value in items.items():
        table.add_row(f"[bold white]•[/] {key}", f"{value}")
...
def start_summary_table(results: dict[ProcessName, StartResult]) -> None:
    ...
    for name, result in results.items():
        ...
        if not result.existed:
            icon = "[bold white]•[/]"
            table.add_row(f"{icon} {label}", "was not started")
            continue

        # Started OK
        if result.ok():
            icon = "[bold green]✔[/]"
            table.add_row(f"{icon} {label}", "started successfully")
            continue

        # Startup failure
        icon = "[bold red]✖[/]"
        table.add_row(f"{icon} {label}", f"failed to start: {result.error}")
